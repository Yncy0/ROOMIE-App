...
export async function uploadFiles(
        ...
        (
            ...
            return new Promise<void>(async (resolve, reject) => {
                ...
                let upload = new Upload(blob, {
                    endpoint:
                        `${process.env.EXPO_PUBLIC_SUPABASE_URL}/storage/v1/upload/resumable`,
                    retryDelays: [0, 3000, 5000, 10000, 20000],
                    headers: {
                        authorization:
                            `Bearer ${process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY}`, // or replace with logged in user's access token.
    ...
    await Promise.allSettled(allUploads as []);
    return;
}
